function [COMTY,ending]=genlouvain1(A,s,xita,cSize)
    ending=0;
    M=A;
    if nnz(M-M')
        M=(M+M')/2;
    ...
    while(isGain==1)
        ...
        %for i=randperm(N)
            ...
            %for j=randperm(length(neighbor_i))
                ...
                if(gain(com_j)==0)
                    ...
                    V_j=sum(cSize(node_cj));
                    Ki_in=2*sum(M(i,node_cj));
                    %gain(com_j)=Ki_in/m - 2*K(i)*sumTot(com_j)/(m*m);
                    %gain(com_j)=(2*(sumIn(com_j)+Ki_in)-sumTot(com_j)-K(i))/(V_j+cSize(i))-...
                    %    ((2*sumIn(com_j)-sumTot(com_j))/V_j+(2*M(i,i)-K(i))/cSize(i));
                    gain(com_j)=((sumIn(com_j)+Ki_in)-xita*(sumTot(com_j)-K(i)))/(V_j+cSize(i))-...
                        ((sumIn(com_j)-xita*sumTot(com_j))/V_j+(M(i,i)-xita*K(i))/cSize(i));
        ...
        Mod=computeMod(com,M,cSize,xita);
    ...
    COMTY.MOD(1) = computeMod(com,M,cSize,xita);
    COMTY.Niter(1) = niter;
    %recursive
    if(s==1)
        newM=M;
        ...
        while(1)
            ...
            [COMt e]=genlouvain1(newM,0,xita,comSize);
            if(e~=1)
                fullCom=zeros(1,N);
                ...
                COMTY.MOD(k) = computeMod(COMfull,M,cSize,xita);
